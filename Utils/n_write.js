const write_process = async (browser, page_naver, now_hour, viewRank, commentRank) => {
    ...
    let title = `${start_time}:00 ~ ${end_time}:00 - TOP 인기글 스마일 봇 - v2.0.1`;
...
`;
    ...
    let notice2 = "(현재 인기글 리스트는 거래게시글 중 [완료]를 제외한 모든 게시글이 대상입니다)" + blank;

    let notice = notice1 + notice2 + blank;
    ...
    await temp_page.goto('https://m.cafe.naver.com/ca-fe/web/cafes/15092639/articles/3403158/comments?fromList=true&menuId=567');
